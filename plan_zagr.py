...
machines = ["Сверлильная", "Фрезерная", "Фрезерная_1", "Фрезерная c ЧПУ", "Токарная", "Токарная с ЧПУ", "Слесарная", "Слесарная_1", "Расточная", "Заготовительная", "Маркирование"]
processing_times = {
    ("Фланец", "O1"): {"Заготовительная": 0.08}, ("Фланец", "O2"): {"Токарная": 1.2, "Токарная с ЧПУ": 1.1}, ("Фланец", "O3"): {"Сверлильная": 0.2, "Фрезерная c ЧПУ": 0.4}, ("Фланец", "O4"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("Фланец", "O5"): {"Маркирование": 0.1},
    ("Прокладка", "O1"): {"Фрезерная": 0.6, "Фрезерная_1": 0.6, "Фрезерная c ЧПУ": 0.7}, ("Прокладка", "O2"): {"Сверлильная": 0.1, "Фрезерная c ЧПУ": 0.3}, ("Прокладка", "O3"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("Прокладка", "O4"): {"Маркирование": 0.1},
    ("Болт", "O1"): {"Заготовительная": 0.08}, ("Болт", "O2"): {"Токарная": 0.6}, ("Болт", "O3"): {"Фрезерная": 0.06, "Фрезерная_1": 0.06, "Фрезерная c ЧПУ": 0.09}, ("Болт", "O4"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("Болт", "O5"): {"Маркирование": 0.1},
    ("Рычаг", "O1"): {"Фрезерная c ЧПУ": 2.3},("Рычаг", "O2"): {"Слесарная": 0.25, "Слесарная_1": 0.25}, ("Рычаг", "O3"): {"Сверлильная": 0.28, "Фрезерная c ЧПУ": 0.4}, ("Рычаг", "O4"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("Рычаг", "O5"): {"Маркирование": 0.1},
    ("Ручка", "O1"): {"Слесарная": 0.5},("Ручка", "O2"): {"Токарная": 1.3}, ("Ручка", "O3"): {"Слесарная": 0.2, "Слесарная_1": 0.2}, ("Ручка", "O4"): {"Маркирование": 0.1},
    ("Рым-болт", "O1"): {"Заготовительная": 0.08}, ("Рым-болт", "O2"): {"Токарная": 0.75, "Токарная с ЧПУ": 0.7}, ("Рым-болт", "O3"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("Рым-болт", "O4"): {"Маркирование": 0.1},
    ("Кница", "O1"): {"Фрезерная": 0.8, "Фрезерная_1": 0.8, "Фрезерная c ЧПУ": 1.0}, ("Кница", "O2"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("Кница", "O3"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("Кница", "O4"): {"Маркирование": 0.1},
    ("ВТУЛКА", "O1"): {"Заготовительная": 0.11}, ('ВТУЛКА', "O2"): {"Токарная": 0.7, "Токарная с ЧПУ": 0.7}, ("ВТУЛКА", "O3"): {"Слесарная": 0.1, "Слесарная_1": 0.1}, ("ВТУЛКА", "O4"): {"Маркирование": 0.1},
    ("РБ", "O1"): {"Токарная": 1.2, "Токарная с ЧПУ": 1.1}, ("РБ", 'O2'): {'Слесарная': 0.1, 'Слесарная_1': 0.1}, ("РБ", "O3"): {"Маркирование": 0.1},
    ('ПРОСТАВЫШ', 'O1'): {'Заготовительная': 0.4}, ('ПРОСТАВЫШ', 'O2'): {'Токарная': 0.7, 'Токарная с ЧПУ': 0.7}, ('ПРОСТАВЫШ', 'O3'): {'Расточная': 1.2}, ('ПРОСТАВЫШ', 'O4'): {'Слесарная': 0.3, 'Слесарная_1': 0.3}, ('ПРОСТАВЫШ', 'O5'): {'Маркирование': 0.1},
    ('ГОЛОВКА', 'O1'): {'Заготовительная': 0.14}, ('ГОЛОВКА', 'O2'): {'Токарная': 0.55, 'Токарная с ЧПУ': 0.6}, ('ГОЛОВКА', 'O3'): {'Фрезерная': 0.55, 'Фрезерная_1': 0.55, "Фрезерная c ЧПУ": 0.5}, ('ГОЛОВКА', 'O4'): {'Слесарная': 0.1, 'Слесарная_1': 0.1}, ('ГОЛОВКА', 'O5'): {'Маркирование': 0.05},
}
...
def main():
    ...
    unique_jobs = list({job for job, _, _, _, _ in schedule_details})
    colors = sns.color_palette("husl", len(unique_jobs))  # Пастельные цвета
    job_color_map = {job: colors[i] for i, job in enumerate(unique_jobs)}

    # --- Step 2: Set up machine indices for Y-axis ---
    machines_1 = []
    for job, op, machine, start, end in schedule_details:
        if machine not in machines_1:
            machines_1.append(machine)
    machine_indices = {m: i for i, m in enumerate(machines_1)}

    # --- Step 3: Plotting Gantt Chart with coloring by job ---
    fig, ax = plt.subplots(figsize=(16, 10))

    for job, op, machine, start, end in schedule_details:
        y_pos = machine_indices[machine]
        color = job_color_map[job]
        ax.barh(y=y_pos, width=end - start, left=start, height=0.4, color=color, edgecolor='black', label=job)
    ...
    ax.invert_yaxis()
    ax.set_xlabel("Время")
    ax.set_title("Диаграмма Ганта: Расписание станков", fontsize=14)

    # Set time ticks on x-axis with step of 0.5
    x_min = min(start for _, _, _, start, _ in schedule_details)
    x_max = max(end for _, _, _, _, end in schedule_details)
    x_ticks = np.arange(x_min, x_max + 0.5, 0.5)
    ax.set_xticks(x_ticks)
    ax.set_xticklabels([f"{t:.1f}" for t in x_ticks], rotation=90)

    # --- Step 5: Add Legend (without duplicates) ---
    handles, labels = ax.get_legend_handles_labels()
    by_label = dict(zip(labels, handles))  # Remove duplicates
    ax.legend(by_label.values(), by_label.keys(), title="Детали", bbox_to_anchor=(1.05, 1), loc='upper left')
